...
public abstract class AbstractCrmProcessor<T, E, M, N> extends AbstractBaseProcessor<T, E, M, N>  {
	
	private static Logger logger = Logger.getLogger(AbstractCrmProcessor.class);
	...
	public String generateRequestXML(M m, ParameterConfig config) throws Exception {
		...
		.append("<crm><head>")
		.append("<api_code><![CDATA[").append(((CrmRequestHeader) m).getApi_code()).append("]]></api_code>")
		.append("<req_serial_no><![CDATA[").append(((CrmRequestHeader) m).getReq_serial_no()).append("]]></req_serial_no>")
		.append("<req_date><![CDATA[").append(((CrmRequestHeader) m).getReq_date()).append("]]></req_date>")
		.append("<security_code><![CDATA[").append(((CrmRequestHeader) m).getSecurity_code()).append("]]></security_code>")
		.append("<security_value><![CDATA[").append(((CrmRequestHeader) m).getSecurity_value()).append("]]></security_value>")
		.append("<sign><![CDATA[").append(((CrmRequestHeader) m).getSign()).append("]]></sign>")
		.append("</head><body>");
		
		requestXml.append(XMLHelper.getXMLFromBean(m));
		requestXml.append("</body></crm>");
	...
	public void verifyResponse(N n, ParameterConfig config) throws ESBException {
		if("SUCCESS".equals(((CrmResponseHeader) n).getReturn_code())) {
			// 签名校验
			String signFromAPIResponse = ((CrmResponseHeader) n).getSign();
