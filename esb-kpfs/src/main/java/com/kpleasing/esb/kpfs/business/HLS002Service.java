...
import com.kpleasing.esb.receive.protocol.kpfs.hls002.ReqHLS002;
import com.kpleasing.esb.receive.protocol.kpfs.hls002.ResHLS002;


public class HLS002Service implements IProcessService {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(HLS002Service.class);

	private void validate(ReqHLS002 req) throws KPFSINDataException {
		// 签名校验
	 ...
	 */
	private void validate2(ResHLS002 res) throws KPFSINDataException {
		// 签名校验
	...
	@Override
	public String process(Map<String, Object> mapHls002) throws KPFSINDataException, KPFSBusinessException {
		ReqHLS002 request = new ReqHLS002();
		try {
			BeanUtils.populate(request, mapHls002);
			...
			ResHLS002 resHls002 = ResHLS002.fromXML(response);
			
			LOGGER.info("前置机返回报文签名校验："+response);
			validate2(resHls002);
			
			resHls002.setReq_serial_no(request.getReq_serial_no());
			resHls002.setReq_date(request.getReq_date());
			resHls002.setRes_serial_no(StringUtil.getSerialNo32());
			resHls002.setRes_date(DateUtil.getCurrentDate(DateUtil.yyyyMMddHHmmss));
			resHls002.setReturn_code("SUCCESS");
			resHls002.setReturn_desc("成功");
			
			return resHls002.toResponseXML();
				
		} catch (IllegalAccessException | InvocationTargetException e) {
			LOGGER.error("HLS002处理异常：" + e.getMessage(), e);
