...
import com.kpleasing.esb.receive.protocol.kpfs.hls008.ReqHLS008;
import com.kpleasing.esb.receive.protocol.kpfs.hls008.ResHLS008;


public class HLS008Service implements IProcessService {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(HLS008Service.class);
	
	
	private void validate(ReqHLS008 req) throws KPFSINDataException {
		// 签名校验
	 ...
	 */
	private void validate2(ResHLS008 res) throws KPFSINDataException {
		// 签名校验
	...
	@Override
	public String process(Map<String, Object> mapHls008) throws KPFSINDataException, KPFSBusinessException {
		ReqHLS008 request = new ReqHLS008();
		try {
			BeanUtils.populate(request, mapHls008);
			...
			ResHLS008 resHls008 = ResHLS008.fromXML(response);
			
			LOGGER.info("前置机返回报文签名校验："+response);
			validate2(resHls008);
			
			resHls008.setReq_serial_no(request.getReq_serial_no());
			resHls008.setReq_date(request.getReq_date());
			resHls008.setRes_serial_no(StringUtil.getSerialNo32());
			resHls008.setRes_date(DateUtil.getCurrentDate(DateUtil.yyyyMMddHHmmss));
			resHls008.setReturn_code("SUCCESS");
			resHls008.setReturn_desc("成功");
			
			return resHls008.toResponseXML();
				
		} catch (IllegalAccessException | InvocationTargetException e) {
			LOGGER.error("HLS008处理异常：" + e.getMessage(), e);
