...
import com.kpleasing.esb.receive.protocol.kpfs.hls007.ReqHLS007;
import com.kpleasing.esb.receive.protocol.kpfs.hls007.ResHLS007;


public class HLS007Service implements IProcessService {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(HLS007Service.class);
	
	
	private void validate(ReqHLS007 req) throws KPFSINDataException {
		// 签名校验
	 ...
	 */
	private void validate2(ResHLS007 res) throws KPFSINDataException {
		// 签名校验
	...
	@Override
	public String process(Map<String, Object> mapHls007) throws KPFSINDataException, KPFSBusinessException {
		ReqHLS007 request = new ReqHLS007();
		try {
			BeanUtils.populate(request, mapHls007);
			...
			ResHLS007 resHls007 = ResHLS007.fromXML(response);
			
			LOGGER.info("前置机返回报文签名校验："+response);
			validate2(resHls007);
			
			resHls007.setReq_serial_no(request.getReq_serial_no());
			resHls007.setReq_date(request.getReq_date());
			resHls007.setRes_serial_no(StringUtil.getSerialNo32());
			resHls007.setRes_date(DateUtil.getCurrentDate(DateUtil.yyyyMMddHHmmss));
			resHls007.setReturn_code("SUCCESS");
			resHls007.setReturn_desc("成功");
			
			return resHls007.toResponseXML();
				
		} catch (IllegalAccessException | InvocationTargetException e) {
			LOGGER.error("HLS007处理异常：" + e.getMessage(), e);
