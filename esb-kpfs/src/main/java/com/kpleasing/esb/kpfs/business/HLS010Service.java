...
import com.kpleasing.esb.receive.protocol.kpfs.hls010.ReqHLS010;
import com.kpleasing.esb.receive.protocol.kpfs.hls010.ResHLS010;


public class HLS010Service implements IProcessService {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(HLS010Service.class);
	
	
	private void validate(ReqHLS010 req) throws KPFSINDataException {
		// 签名校验
	 ...
	 */
	private void validate2(ResHLS010 res) throws KPFSINDataException {
		// 签名校验
	...
	@Override
	public String process(Map<String, Object> mapHls010) throws KPFSINDataException, KPFSBusinessException {

		ReqHLS010 request = new ReqHLS010();
		try {
			BeanUtils.populate(request, mapHls010);
			...
			ResHLS010 resHls010 = ResHLS010.fromXML(response);
			
			LOGGER.info("前置机返回报文签名校验："+response);
			validate2(resHls010);
			
			resHls010.setReq_serial_no(request.getReq_serial_no());
			resHls010.setReq_date(request.getReq_date());
			resHls010.setRes_serial_no(StringUtil.getSerialNo32());
			resHls010.setRes_date(DateUtil.getCurrentDate(DateUtil.yyyyMMddHHmmss));
			resHls010.setReturn_code("SUCCESS");
			resHls010.setReturn_desc("成功");
			
			return resHls010.toResponseXML();
				
		} catch (IllegalAccessException | InvocationTargetException e) {
			LOGGER.error("HLS010处理异常：" + e.getMessage(), e);
