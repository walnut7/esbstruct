...
import com.kpleasing.esb.receive.protocol.kpfs.hls004.ReqHLS004;
import com.kpleasing.esb.receive.protocol.kpfs.hls004.ResHLS004;


public class HLS004Service implements IProcessService {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(HLS004Service.class);

	private void validate(ReqHLS004 req) throws KPFSINDataException {
		// 签名校验
	 ...
	 */
	private void validate2(ResHLS004 res) throws KPFSINDataException {
		// 签名校验
	...
	@Override
	public String process(Map<String, Object> mapHls004) throws KPFSINDataException, KPFSBusinessException {
		ReqHLS004 request = new ReqHLS004();
		try {
			BeanUtils.populate(request, mapHls004);
			...
			ResHLS004 resHls004 = ResHLS004.fromXML(response);
			
			LOGGER.info("前置机返回报文签名校验："+response);
			validate2(resHls004);
			
			resHls004.setReq_serial_no(request.getReq_serial_no());
			resHls004.setReq_date(request.getReq_date());
			resHls004.setRes_serial_no(StringUtil.getSerialNo32());
			resHls004.setRes_date(DateUtil.getCurrentDate(DateUtil.yyyyMMddHHmmss));
			resHls004.setReturn_code("SUCCESS");
			resHls004.setReturn_desc("成功");
			
			return resHls004.toResponseXML();
				
		} catch (IllegalAccessException | InvocationTargetException e) {
			LOGGER.error("HLS004处理异常：" + e.getMessage(), e);
