...
import com.kpleasing.esb.receive.protocol.kpfs.hls005.ReqHLS005;
import com.kpleasing.esb.receive.protocol.kpfs.hls005.ResHLS005;


public class HLS005Service implements IProcessService {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(HLS005Service.class);
	
	private void validate(ReqHLS005 req) throws KPFSINDataException {
		// 签名校验
	 ...
	 */
	private void validate2(ResHLS005 res) throws KPFSINDataException {
		// 签名校验
	...
	@Override
	public String process(Map<String, Object> mapHls005) throws KPFSINDataException, KPFSBusinessException {
		ReqHLS005 request = new ReqHLS005();
		try {
			BeanUtils.populate(request, mapHls005);
			...
			ResHLS005 resHls005 = ResHLS005.fromXML(response);
			
			LOGGER.info("前置机返回报文签名校验："+response);
			validate2(resHls005);
			
			resHls005.setReq_serial_no(request.getReq_serial_no());
			resHls005.setReq_date(request.getReq_date());
			resHls005.setRes_serial_no(StringUtil.getSerialNo32());
			resHls005.setRes_date(DateUtil.getCurrentDate(DateUtil.yyyyMMddHHmmss));
			resHls005.setReturn_code("SUCCESS");
			resHls005.setReturn_desc("成功");
			
			return resHls005.toResponseXML();
				
		} catch (IllegalAccessException | InvocationTargetException e) {
			LOGGER.error("HLS005处理异常：" + e.getMessage(), e);
