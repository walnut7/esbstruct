...
import com.kpleasing.esb.receive.protocol.kpfs.hls006.ReqHLS006;
import com.kpleasing.esb.receive.protocol.kpfs.hls006.ResHLS006;


public class HLS006Service implements IProcessService {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(HLS006Service.class);
	
	private void validate(ReqHLS006 req) throws KPFSINDataException {
		// 签名校验
	 ...
	 */
	private void validate2(ResHLS006 res) throws KPFSINDataException {
		// 签名校验
	...
	@Override
	public String process(Map<String, Object> mapHls006) throws KPFSINDataException, KPFSBusinessException {
		ReqHLS006 request = new ReqHLS006();
		try {
			BeanUtils.populate(request, mapHls006);
			...
			ResHLS006 resHls006 = ResHLS006.fromXML(response);
			
			LOGGER.info("前置机返回报文签名校验："+response);
			validate2(resHls006);
			
			resHls006.setReq_serial_no(request.getReq_serial_no());
			resHls006.setReq_date(request.getReq_date());
			resHls006.setRes_serial_no(StringUtil.getSerialNo32());
			resHls006.setRes_date(DateUtil.getCurrentDate(DateUtil.yyyyMMddHHmmss));
			resHls006.setReturn_code("SUCCESS");
			resHls006.setReturn_desc("成功");
			
			return resHls006.toResponseXML();
				
		} catch (IllegalAccessException | InvocationTargetException e) {
			LOGGER.error("HLS006处理异常：" + e.getMessage(), e);
